...
class HeteroscedasticGaussianMSE(nn.Module):
    ...
    def __init__(self, n_min=0.09, n_max=1.0, one_sided=False):
        super().__init__()
        assert 0 < n_min < n_max
        self.n_min = n_min
        self.n_max = n_max
        # only forgive pixels where pred > soft_gt (the "missing annotation" direction);
        # pixels with pred < soft_gt (model under-shoots a KEPT label) keep full supervision
